...
function [y, theta, M_z, V_y, T, F] = gen_shear_bending(x, D, d, E, do_plot)
    ...
    theta_tmp = cumtrapz(x/1000, M_over_EI);    % +C
    y_tmp = cumtrapz(x/1000, theta_tmp);        % +Cx + D
    
    % Get the correct constant: D = 0 since y(x=0) = 0
    % Get the constant that makes: y(x=0.45) = 0
    C = -y_tmp(x2ind(x, 0.45)) / 0.45;
    theta = theta_tmp + C;
    y = y_tmp + C*x;
    ...
    if do_plot
        ...
        xlabel('x (mm)'); ylabel('M_z (Nm)'); title('Bending Moment vs. x');
        axis([-50, 650, -400, 400]);
